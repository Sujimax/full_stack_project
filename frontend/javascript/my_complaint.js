document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    const response = await fetch("http://127.0.0.1:8000/complaints/me", {
      headers: {
    ...
    complaints.forEach((complaint) => {
      ...
      complaintBox.innerHTML = `
        <div class="complaint-content">
          <div class="details">
            ...
            <p><strong>Status:</strong> <span class="${statusClass}">${complaint.status || "Pending"}</span></p>
          ...
          <div class="image">
            <img src="${complaint.image_url ? "http://127.0.0.1:8000/" + complaint.image_url : "../images/icon1.png"}" alt="Complaint Image">
          </div>
        </div>
      `;
      ...
      complaintBox.querySelector(".delete-btn").addEventListener("click", async () => {
        ...
        const res = await fetch(`http://127.0.0.1:8000/complaints/${complaint.id}`, {
          method: "DELETE",
          headers: {
            "Authorization": `Bearer ${token}`
          }
